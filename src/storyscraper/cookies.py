...
secretstorage: Any | None
_SECRETSTORAGE_UNAVAILABLE_REASON: str | None
try:
    secretstorage = importlib.import_module("secretstorage")
except ImportError:  # pragma: no cover - optional dependency
    secretstorage = None
    _SECRETSTORAGE_UNAVAILABLE_REASON = (
        "because the optional 'secretstorage' dependency is not installed. "
        "Install it with `pip install secretstorage`."
    )
else:
...
class DownloadError(Exception):
    """Error raised when a fatal cookies extraction issue occurs."""


class YoutubeDLError(Exception):
    """Compatibility exception used by the vendored yt-dlp cookies module."""


class _SimpleLogger:
    """Lightweight logger compatible with yt-dlp's expectations."""

    def __init__(self) -> None:
        self._logger = logging.getLogger("storyscraper.cookies")

    def info(self, message: str) -> None:  # pragma: no cover - logging passthrough
        self._logger.info(message)

    def debug(self, message: str) -> None:  # pragma: no cover - logging passthrough
        self._logger.debug(message)

    def warning(
        self, message: str, only_once: bool = False
...
class _NullProgress:
    """No-op context manager used instead of minicurses progress bars."""
    ...
    def print(self, message: str) -> None:  # pragma: no cover - intentionally blank
        del message


class _SimplePopen:
    """Shim around subprocess.run() mimicking yt-dlp's helper."""
...
def _error_to_str(exc: BaseException) -> str:
    return str(exc)
...
def _is_path_like(value: Any) -> bool:
    return isinstance(value, (os.PathLike, str))
...
def _str_or_none(value: Any, default: str | None = None) -> str | None:
...
def _write_string(message: str) -> None:  # pragma: no cover - logging helper
...
def aes_gcm_decrypt_and_verify_bytes(
    data: bytes, key: bytes, tag: bytes, nonce: bytes
) -> bytes:
...
def unpad_pkcs7(data: bytes) -> bytes:
    padding = data[-1]
    if not 0 < padding <= len(data):
        raise ValueError("Invalid PKCS#7 padding")
    return data[:-padding]


Popen = _SimplePopen
error_to_str = _error_to_str
expand_path = _expand_path
is_path_like = _is_path_like
sanitize_url = _sanitize_url
